...
// Function to notify canvas clear events (Replacing `any` with proper window interface)
 ...
 *
 * A collaborative drawing canvas that supports real-time synchronization between multiple users.
 * This component handles both local drawing operations and remote updates, including:
 *
 * Features:
 * - Real-time stroke synchronization with optimistic local rendering
 * - Partial stroke updates during drawing for smooth collaboration
 * - Remote cursor tracking for all connected users
 * - Support for brush and eraser tools with configurable properties
 * - Canvas clearing with synchronization across all clients
 *
 * Implementation Notes:
 * - Uses a dual-canvas approach: one for drawing strokes, one for rendering cursors
 * - Implements throttled updates based on syncInterval to balance responsiveness and network load
 * - Maintains sequence tracking to handle out-of-order message delivery
 * - Deduplicates partial stroke updates to prevent rendering artifacts
 * - Support remote brush properties (color, size, lineCap) for consistent rendering
 */
  ...
  (
    ...
    useEffect(() => {
      ...
      remotePartialStrokes.forEach(partialStroke => {
        const { strokeId, points, brushColor, brushSize, lineCap, sequence, isEraser } =
          partialStroke;
    ...
    useEffect(() => {
      ...
      return () => {
        if (typeof window !== 'undefined') {
          const customWindow = window as unknown as CustomWindow;
          delete customWindow.notifyCanvasClearEvent;
        }
      };
    }, []);
    ...
    const updateCustomCursorPosition = useCallback(
      (x: number, y: number) => {
