...
import { SupabaseClient, RealtimeChannel, RealtimePresenceState } from '@supabase/supabase-js';
import { Stroke, CursorPosition, PartialStroke } from '../components/CanvasBoard';

type ConnectionStatus = 'connecting' | 'connected' | 'disconnected';
type PresenceState = RealtimePresenceState<{
  userId: string;
  username: string;
  online_at: string;
  cursorColor: string;
...
const clearAllStrokes = async (supabase: SupabaseClient, roomId: string) => {
  try {
    ...
    const { error } = await supabase.rpc('clear_room_strokes', { p_room_id: roomId });
...
// Define a proper interface for the CustomWindow
interface CustomWindow extends Window {
  notifyCanvasClearEvent?: () => void;
}

// Define a type for the presence events
...
/**
 * Custom hook for managing real-time collaboration through Supabase
 *
 * This hook encapsulates all the real-time synchronization logic for the collaborative canvas,
 * managing the communication layer between multiple clients and the persistence layer.
 *
 * Key Features:
 * 1. User presence management - tracks online users and their information
 * 2. Cursor position synchronization - broadcasts and receives cursor positions
 * 3. Stroke synchronization - both partial (in-progress) and complete strokes
 * 4. Canvas clearing synchronization - propagates clear actions to all clients
 * 5. Database persistence - saves completed strokes to the database for persistence
 *
 * Implementation Details:
 * - Uses Supabase Realtime channels for pub/sub functionality
 * - Implements throttling for cursor and partial stroke updates to optimize performance
 * - Handles connection state management and reconnection logic
 * - Maintains local state for remote strokes, cursors, and partial strokes
 * - Deduplicates incoming updates to prevent rendering issues
 *
 * @param supabase - Supabase client instance
 * @param options - Configuration options including user info and room settings
...
export function useRealtimeCollaboration(
  ...
  const [currentCursorSyncInterval, setCurrentCursorSyncInterval] =
    useState<number>(cursorSyncInterval);
    ...
    newChannel
      ...
      .on('broadcast', { event: 'clear' }, () => {
        ...
        if (typeof window !== 'undefined') {
          ...
          if (customWindow.notifyCanvasClearEvent) {
            customWindow.notifyCanvasClearEvent();
          }
        }
      ...
      .on('presence', { event: 'leave' }, ({ key, leftPresences }: PresenceLeaveEvent) => {
  ...
  }, [supabase, userId, userName, userColor, roomId, connectionStatus]);
  ...
  const sendCompleteStroke = useCallback(
    (stroke: Stroke) => {
      if (channel && isConnected) {
  ...
  const sendClearCanvas = useCallback(() => {
    if (channel && isConnected) {
      ...
      clearAllStrokes(supabase, roomId).catch(error => {
